...
func TestTLSInfoEmpty(t *testing.T) {
	tests := []struct {
		...
		{TLSInfo{}, true},
		{TLSInfo{TrustedCAFile: "foo"}, true},
		{TLSInfo{CertFile: "foo"}, false},
		{TLSInfo{KeyFile: "bar"}, false},
		{TLSInfo{CertFile: "foo", KeyFile: "bar"}, false},
		{TLSInfo{CertFile: "foo", KeyFile: "baz"}, false},
		{TLSInfo{CertFile: "bar", TrustedCAFile: "bza"}, false},
		{TLSInfo{CertFile: "foo", KeyFile: "bar", TrustedCAFile: "bza"}, false},
	}
	for i, tt := range tests {
		got := tt.info.Empty()
		if tt.want != got {
